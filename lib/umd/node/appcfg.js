...
function obj_deepExtend(target, source, opts) {
    ...
    for (var key in source) {
        ...
        if ((0, atma_utils_1.is_Array)(val)) {
            var extendArrays = (opts === null || opts === void 0 ? void 0 : opts.extendArrays) !== false;
            var idKey = (_a = opts === null || opts === void 0 ? void 0 : opts.mergeArrayItems) === null || _a === void 0 ? void 0 : _a[key];
...
function mergeArrays(targetArr, sourceArr, idKey) {
    var targetDict = targetArr.reduce(function (dict, item) {
        dict[item[idKey]] = item;
        return dict;
    }, {});
    var sourceDict = sourceArr.reduce(function (dict, item) {
        dict[item[idKey]] = item;
        return dict;
    }, {});
    for (var i = 0; i < targetArr.length; i++) {
        var target = targetArr[i];
        var source = sourceDict[target[idKey]];
        if (source != null) {
            targetArr[i] = __assign(__assign({}, target), source);
        }
    }
    for (var i = 0; i < sourceArr.length; i++) {
        var source = sourceArr[i];
        var target = targetDict[source[idKey]];
        if (target == null) {
...
function obj_interpolate(obj, root, isOptional) {
    ...
    obj_visitStrings(obj, function (str, key, parent) {
        ...
        for (var i = 1; i < arr.length; i++) {
            var slug = arr[i];
            var endIdx = slug.indexOf('}');
            if (endIdx === -1) {
                return str;
            }
            var property = slug.substring(0, endIdx).trim();
            var value = getValueOuter(property, obj, isOptional);
            var rest = slug.substring(endIdx + 1).trim();
            if (rest[0] === '}') {
...
;
function getValueOuter(property, obj, isOptional) {
    var val = getValue(property, obj, isOptional);
    if (val == null && isOptional !== true) {
        (0, log_1.log_warn)('<config: obj_interpolate: property not exists in root', property);
    }
    return val !== null && val !== void 0 ? val : ('${{' + property + '}}');
}
function getValue(property, obj, isOptional) {
    if (obj == null) {
        return null;
    }
    if (typeof process !== 'undefined' && process.env != null) {
        var val_1 = (0, atma_utils_1.obj_getProperty)(process.env, property);
        if (val_1 != null) {
            return val_1;
        }
    }
    var val = (0, atma_utils_1.obj_getProperty)(obj, property);
    if (val != null) {
        return val;
    }
    if (!isOptional) {
 ...
 */
function cfg_merge(target, config, setterProperty, additionalParams) {
    if (config == null) {
        return;
    }
    if (setterProperty) {
    ...
    (0, object_1.obj_deepExtend)(target, config, additionalParams);
}
...
var Sources = /** @class */ (function () {
    ...
    Sources.prototype.loadSync = function (rootConfig) {
        ...
        while (++i < imax) {
            ...
            (0, cfg_1.cfg_merge)(rootConfig, source.config, source.data.setterProperty, source.data);
            if (after) {
...
var yaml;
(function (yaml) {
    function parse(str) { return parser.parse(str); }
    yaml.parse = parse;
    function serialize(mix) { return serializer.serialize(mix); }
    yaml.serialize = serialize;
    var parser;
    (function (parser) {
        var YamlError = /** @class */ (function (_super) {
            __extends(YamlError, _super);
            function YamlError(message, line) {
                var _this = _super.call(this, line == null ? message : "".concat(message, " at line ").concat(line.number)) || this;
                _this.name = 'YamlError';
                return _this;
            }
            return YamlError;
        }(Error));
        function parse(input) {
            ...
            var value = parseBlock(state, lines[0].indent);
            if (state.index < lines.length)
                throw new YamlError('Unexpected content', lines[state.index]);
            return value;
        }
        parser.parse = parse;
        function tokenize(input) {
            var result = [];
            var blockParentIndent = null;
            input.replace(/\r\n?/g, '\n').split('\n').forEach(function (raw, i) {
                var number = i + 1;
                if (blockParentIndent != null) {
                    var indent = getIndent(raw);
                    if (raw.trim().length === 0 || indent > blockParentIndent) {
                        result.push({ raw: raw, text: raw.trimRight(), indent: indent, number: number });
                        return;
                    }
                    blockParentIndent = null;
                }
                var text = stripComment(raw).trimRight();
                var trimmed = text.trim();
                if (trimmed.length === 0 || trimmed === '---' || trimmed === '...')
                    return;
                var line = { raw: raw, text: text, indent: getIndent(text), number: number };
                result.push(line);
        ...
        function parseBlock(state, indent) {
            ...
            if (line.indent !== indent)
                throw new YamlError('Unexpected indentation', line);
            var text = line.text.slice(indent);
            if (text.charAt(0) === '-')
        ...
        function parseObject(state, indent, target) {
            var obj = target || {};
            while (state.index < state.lines.length) {
                ...
                if (text.charAt(0) === '-')
                    break;
                var pair = readKeyValue(text, line);
                state.index++;
        ...
        function parseArray(state, indent) {
            ...
            while (state.index < state.lines.length) {
                ...
                if (text.charAt(0) !== '-')
                    break;
                if (text.length > 1 && /\S/.test(text.charAt(1)))
                    throw new YamlError('A sequence marker must be followed by whitespace', line);
                var value = text.slice(1).trimLeft();
                state.index++;
                if (value.length === 0) {
                    arr.push(readNestedValue(state, indent));
                    continue;
                }
                if (isPlainKeyValue(value)) {
                    var pair = readKeyValue(value, line);
                    var obj = {};
                    obj[pair.key] = readNodeValue(state, indent, pair.value, line);
                    var next = state.lines[state.index];
                    if (next != null && next.indent > indent)
                        parseObject(state, next.indent, obj);
                    arr.push(obj);
                    continue;
                }
        ...
        function readNodeValue(state, parentIndent, value, line) {
            value = value.trim();
            if (value.length === 0)
                return readNestedValue(state, parentIndent);
            if (value === '|' || value === '>')
                return readBlockScalar(state, parentIndent, value === '>');
            return readInlineOrScalar(value, line);
        }
        function readNestedValue(state, parentIndent) {
            var next = state.lines[state.index];
            if (next == null || next.indent <= parentIndent)
                return null;
            return parseBlock(state, next.indent);
        }
        function readBlockScalar(state, parentIndent, folded) {
            var rows = [], blockIndent = null;
            while (state.index < state.lines.length) {
                var line = state.lines[state.index];
                if (line.text.trim().length !== 0 && line.indent <= parentIndent)
                    break;
                if (blockIndent == null && line.text.trim().length > 0)
                    blockIndent = line.indent;
                var cut = blockIndent == null ? parentIndent + 1 : blockIndent;
                rows.push(line.raw.slice(Math.min(cut, line.raw.length)));
                state.index++;
            }
            if (folded === false)
                return rows.join('\n');
            return rows.reduce(function (out, row) { return row.trim().length === 0 ? out + '\n' : out + (out.length === 0 || /\n$/.test(out) ? '' : ' ') + row.trim(); }, '');
        }
        function readInlineOrScalar(value, line) {
            if (value.charAt(0) === '[')
                return readFlowArray(value, line);
            if (value.charAt(0) === '{')
                return readFlowObject(value, line);
            return coerce(value, line);
        }
        function readFlowArray(value, line) {
            var inner = unwrapFlow(value, '[', ']', line);
            return inner.trim().length === 0 ? [] : splitFlow(inner, ',').map(function (part) { return readInlineOrScalar(part.trim(), line); });
        }
        function readFlowObject(value, line) {
            var inner = unwrapFlow(value, '{', '}', line);
            var obj = {};
            if (inner.trim().length === 0)
                return obj;
            splitFlow(inner, ',').forEach(function (part) {
                var pair = readKeyValue(part.trim(), line);
                obj[pair.key] = readInlineOrScalar(pair.value.trim(), line);
            });
            return obj;
        }
        function unwrapFlow(value, open, close, line) {
            value = value.trim();
            if (value.charAt(0) !== open || value.charAt(value.length - 1) !== close)
                throw new YamlError("Expected ".concat(open).concat(close, " flow value"), line);
            return value.slice(1, -1);
        }
        function readKeyValue(text, line) {
            var index = indexOfToken(text, ':');
            if (index === -1)
                throw new YamlError('Expected key/value pair', line);
            var key = text.slice(0, index).trim();
            if (key.length === 0)
                throw new YamlError('Expected key', line);
            return { key: unquote(key, line), value: text.slice(index + 1) };
        }
        function isPlainKeyValue(value) {
            var first = value.charAt(0);
            return first !== '[' && first !== '{' && first !== '"' && first !== "'" && indexOfToken(value, ':') !== -1;
        }
        function splitFlow(value, delimiter) {
        ...
        function unquote(value, line) {
            ...
            return value.replace(/\\(["\\/bfnrt])/g, function (_, c) {
                switch (c) {
                    case 'b': return '\b';
                    case 'f': return '\f';
                    case 'n': return '\n';
    ...
    (function (serializer) {
        function serialize(mix, indent) {
            ...
            if (typeof mix === 'string')
                return serializeStringValue(mix, indent);
            if (Array.isArray(mix)) {
                if (mix.length === 0)
                    return '[]';
                return mix.map(function (item) {
                    if (item != null && typeof item === 'object') {
                        return "".concat(indentation, "-\n").concat(doIndent(indent + 1)).concat(serialize(item, indent + 1).replace(/^\s+/, ''));
                    }
                    return "".concat(indentation, "- ").concat(serialize(item, indent + 1).trim());
                }).join('\n');
            }
            if (typeof mix === 'object') {
                ...
                return keys.map(function (key) {
                    ...
                    if (shouldStartFromNewLine(value)) {
                        return "".concat(indentation).concat(serializeKey(key), ":\n").concat(doIndent(indent + 1)).concat(serialize(value, indent + 1).replace(/^\s+/, ''));
                    }
                    return "".concat(indentation).concat(serializeKey(key), ": ").concat(serialize(value, indent + 1).trim());
                }).join('\n');
            }
            return String(mix);
        }
        ...
        function serializeKey(key) {
            return /^[A-Za-z0-9_\-]+$/.test(key)
                ? key
                : "\"".concat(key.replace(/\\/g, '\\\\').replace(/"/g, '\\"'), "\"");
        }
        function serializeStringValue(value, indent) {
            if (value.indexOf('\n') !== -1) {
                return "|\n".concat(doIndent(indent + 1)).concat(value.split('\n').join("\n".concat(doIndent(indent + 1))));
            }
    ...
    function doIndent(count) { return '  '.repeat(count); }
...
SourceFactory_1.SourceFactory.register('files', {
    ...
    create: function (data) {
        return data.files.map(function (file) {
            var info = typeof file === 'string' ? { path: file } : file;
            return SourceFactory_1.SourceFactory.create(info).toArray();
...
SourceFactory_1.SourceFactory.register('directory', {
    ...
    create: function (data) {
        ...
        var files = dir
            ...
            return __assign(__assign({}, data), { path: path });
        });
